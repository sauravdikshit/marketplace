...
import { Appbar, Text, useTheme,Badge } from "react-native-paper";
...
const MenClothing = () => {
  ...
  const getData = async (text) => {
    try {
      const response = await GetDataApi.getMenClothData();
  ...
  return (
    ...
    >
      <Appbar className=" "  style={{ height: 50, backgroundColor: theme.colors.background}}>
        <Appbar.BackAction
          onPress={() => {
            navigation.navigate("BottomNav");
          }}
        />
        <Appbar.Content title="Men Fashion" titleStyle={{ fontSize: 18 }} />
